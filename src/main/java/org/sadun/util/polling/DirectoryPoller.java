...
/**
 * Copyright (C) 1999, 2000 Cristiano Sadun. This software is released under LGPL. A Thread class to periodically poll the contents of one or more directories. External asynchronous processes may put some files in one of the controlled directories, and the poller can be used to detect their arrival.
 * <p>
 * Once started, this thread polls one or more directories every <i>x</i> milliseconds, (see {@link DirectoryPoller#setPollInterval(long) setPollInterval()} and {@link DirectoryPoller#getPollInterval() getPollInterval()}), looking for new files satisfying a given filter. <br>
 * Optionally, the thread may start by going to sleep (if {@link DirectoryPoller#setStartBySleeping(boolean) setStartBySleeping()} is invoked).
 * <p>
 ...
 * <p>
 * Without time-based mode, on every pass the poller will select <i>all</i> the files in a controlled directory that match the filter, so it's client code responsability to phisically move the files already processed somewhere else (see also the <a href="#automove">automove</a> mode below).
 * <p>
 * The poller always notifies an event for a set of file discovered in a particular directory. It may optionally notify a separate event for each file (see {@link DirectoryPoller#setSendSingleFileEvent(boolean) setSendSingleFileEvent()} and {@link DirectoryPoller#isSendSingleFileEvent() isSendSingleFileEvent()}).
 * <p>
 * The following events (all time-stamped) are produced:
 * <ul>
 * <li>{@link CycleStartEvent CycleStartEvent} <br>
 * The poller has awaken, and is starting to look in the directory set
 * <li>{@link DirectoryLookupStartEvent DirectoryLookupStartEvent} <br>
 * The poller is starting to look into a specific directory
 * <li>{@link FileSetFoundEvent FileSetFoundEvent} <br>
 * The poller has found some files in a directory, which match the polling criteria
 * <li>{@link FileFoundEvent FileFoundEvent} (<i>optional</i>) <br>
 * The poller has found one file in a directory, which match the polling criteria
 * <li>{@link DirectoryLookupEndEvent DirectoryLookupStartEvent} <br>
 * The poller has finished to look a specific directory
 * <li>{@link CycleEndEvent CycleEndEvent} <br>
 ...
 * <p>
 * If any exception occurs during the move operation, the relevant methods of {@link PollManager PollManager} are invoked (as a consequence of the notification of an underlying {@link com.deltax.util.listener.ExceptionSignal com.deltax.util.listener.ExceptionSignal}).
 * <p>
 * If autoMove mode is enabled (see {@link DirectoryPoller#setAutoMove(boolean) setAutoMove()}) an arbitary directory may be explicitly associated to one of the controlled directories by using {@link DirectoryPoller#setAutoMoveDirectory(java.io.File, java.io.File) setAutoMoveDirectory()}.
 * <p>
...
public class DirectoryPoller extends BaseSignalSourceThread implements Terminable
{
	...
	public void addDirectory(File dir)
	{
		...
		for (int i = 0; i < originalDirs.length; i++)
			if (originalDirs[i].getAbsoluteFile().equals(dir.getAbsoluteFile()))
				return;
	...
	/**
	 * Instruct the poller to automatically move the file to the directory associated to each directory under control, which can be set/retrieved by {@link DirectoryPoller#setAutoMoveDirectory(java.io.File, java.io.File) setAutoMoveDirectory()}/{@link DirectoryPoller#getAutoMoveDirectory(java.io.File) getAutoMoveDirectory()} (see also class description).
	 * @param v if <b>true</b>, the poller will automatically move selected files in the "received" directory associated to each directory under control
	 */
	...
	/**
	 * Returns the directory associated to the given controlled directory, where files polled are automatically moved if the autoMove mode is active (see {@link DirectoryPoller#setAutoMove(boolean) setAutoMove()} and class description).
	 * <p>
	 ...
	 * <p>
	 * For the call to have any meaning, the poller must be in time-based mode, that is, {@link DirectoryPoller#setTimeBased(boolean) setTimeBased()} must have been called with <b>true</b> as parameter (se class comment): if the parameter is <b>true</b>, the poller will select only files older than the last polling time (besides applying any user-defined filter); if the parameter is <b>false</b>, the poller will select only files <i>whose last modification time is higher than the higher last modification time found in the last polling cycle</i>.
	...
	/**
	 * Return <b>true</b> if the poller is currently instructed to send per-file events to the listeners (see class comment).
