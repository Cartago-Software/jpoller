...
/**
 * Copyright (C) 1999, 2000 Cristiano Sadun. This software is released under LGPL. Clients implementing this interface and registering with {@link DirectoryPoller#addPollManager(org.sadun.util.polling.PollManager) DirectoryPoller.addPollManager()} receive method calls whenever a polling event occur.
 * @author C. Sadun
 * @version 1.0
 */
public interface PollManager
{
	...
	/**
	 * Invoked for each of the files found, if {@link DirectoryPoller#setSendSingleFileEvent(boolean) setSendSingleFileEvent()} has been invoked before starting the poller thread.
	 * @param evt the file set found event
	 */
