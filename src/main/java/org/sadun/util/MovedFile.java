...
/**
 * Copyright (C) 1999, 2000 Cristiano Sadun. This software is released under LGPL. A class to encapsulate the concept of a file which has been moved from a path to another, which simplify inversion of the operation.
 * <p>
 * The class provides an auto-synchronization mode (disable by default) which checks the paths with actual files on the filesystem before performing an operation.
 * <p>
 * The check will fail, however, if files for both original and destination path do exist (i.e. if a copy has taken place).
 * @author C. Sadun
 * @version 1.0
 */
public class MovedFile
{
	...
	/**
	 * Create a MovedFile object, with the given original and destination paths, and whose state is automatically detected.
	 * @param original the original path for the move operation
	 * @param destination the destination path for the move operation
	 */
	...
	/**
	 * Revert the move operation. If the file had been moved, it's restored in its original position. If it's already reverted, a RuntimeException will be raised.
	...
	public synchronized boolean revert()
	{
		if (autoSync)
			sync();
		if (isOriginal)
			throw new RuntimeException("File already reverted:" + original.getAbsolutePath());
		return invert0();
	}
	
	/**
	 * Perform again the move operation. If the file had been reverted, it's moved in its previous "moved" position. If it's already moved, a RuntimeException will be raised.
	 * @exception RuntimeException if the file is in wrong state
	 * @return <b>true</b> if the operation is successful.
	 */
	public synchronized boolean moveAgain()
	{
		if (autoSync)
			sync();
		if (!isOriginal)
			throw new RuntimeException("File already moved:" + destination.getAbsolutePath());
		return invert0();
	}
	
	/**
	 * Synchronize the state of the object with the filesystem
	 * @exception RuntimeException if a filesystem file exists for both original and destination paths.
	 */
	public void sync()
	{
	...
	/**
	 * Return the destination path of the file. Since the file has been moved, this File object corresponds to the physical file on the filesystem.
